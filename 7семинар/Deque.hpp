...
public:
    ...
    inline DequeIterator begin() const {
        Stored *first = // возвращаем указатель на первый реально существующий элемент
                (head_of_ring == end_of_buffer?
                 ...
                 //--<≡===>--
                 head_of_ring + 1);
        return DequeIterator(first, beginning_of_buffer, end_of_buffer);
    }

    inline DequeIterator end() const {
    ...
    Deque<Stored> &operator+=(Deque<Stored> const &other);
...
//объекта в этом массиве. Но для формируемого массива elongate() вызываться не должен
template <typename Stored>
Deque<Stored>::Deque(const Deque &old_deque){
...
//без копирования не обойтись.
template <typename Stored>
void Deque<Stored>::push_front(const Stored &in_value){
...
template <typename Stored>
void Deque<Stored>::push_back(const Stored &in_value){
    // обычный режим - просто добавили значение в массив,
    // --<====>-----  [≡]>  --<====≡>----
    // ==>-----<====  [≡]>  ==≡>----<====
...
//https://mbevin.wordpress.com/2012/11/14/range-based-for/ - используемый материал
template <typename Stored>
class Deque<Stored>::DequeIterator{ // итератор маскируется под указатель.
...
//теперь я хочу переопределить +=, +, =.
template <typename Stored>
Deque<Stored> &Deque<Stored>::operator+=(Deque<Stored> const &other){
...
template <typename Stored>
void Deque<Stored>::elongate(){//увеличение в 2 раза размера буффера.
...
template <typename Stored>
u_int64_t Deque<Stored>::get_number_of_elements()const {
