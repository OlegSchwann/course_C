...
max по i ∑ по j | a[i][j] |
Формат хранения матриц в файлах – неразреженный – в первой строке файла записано количество строк матрицы; во второй – количество столбцов матрицы; в третьей – количество ненулевых элементов матрицы; в последующих строках файла записаны индексы и значения этих ненулевых элементов матрицы (информация о каждом элементе матрицы записана на отдельной строке) в следующей последовательности: номер_строки, номер_столбца, значение_элемента. Считать, что пары индексов элементов матрицы в файле упорядочены по возрастанию. В качестве внутреннего представления матрицы в памяти программы использовать двумерный массив вещественных чисел. Целевой алгоритм не должен зависеть от представления матрицы в файле и в памяти, т.е. для работы с матрицей должны быть реализованы и использованы в алгоритме функции: double get_elem(void *matr, int row, int col), которая возвращает значение элемента матрицы по его индексам, и void set_elem(void *matr, int row, int col, double elem), которая устанавливает значение элемента матрицы по его индексам.*/
...
//создаём для доступа matrix[m_line][m_column] = elem;
double **create_matrix(int m_column, int m_line){ //выделяет память или возвращает NULL в случае ошибки
    ...
    double **result = (double **)calloc((size_t)m_line, sizeof(double *));
    if (result == NULL){
        fprintf(stderr, "No ability allocate memory\n");
        return NULL;
    }
    for(int i = 0; i < m_line; i++){
        result[i] = (double *)calloc((size_t)m_column, sizeof(double));
        if (result[i] == NULL){
            fprintf(stderr, "No ability allocate memory\n");
            return NULL;
        }
...
void dell_matrix(double **matrix, int m_column){
...
double get_elem(void *matr, int m_line, int m_column){
    //возвращает значение элемента матрицы по его индексам;
    double **matrix = matr;
    return matrix[m_line][m_column]; //"слишком просто, что бы быть правдой!" - но, как оказалось, это так и работает.
}

void set_elem(void *matr, int m_line, int m_column, double elem){
    //устанавливает значение элемента матрицы по его индексам
    double **matrix = matr;
    matrix[m_line][m_column] = elem;
}
...
int main(int argc, char * argv[]) {
    ...
    for(int i = 0; i < m_line; i++){
        ...
        for(int j = 0; j < m_column-1; j++){
            printf("%.1f, ", get_elem(matrix, i, j));
        }
        printf("%.1f", get_elem(matrix, i, m_column-1));
        printf("]\n");
    }

    printf("Макс-норма матрицы = %.2lf", matrix_max_norm(matrix, m_line, m_column));
    fflush(stdout);
    dell_matrix(matrix, m_line);
    return 0;
}
